...
class TextCtrlAutoComplete(wx.TextCtrl, listmix.ColumnSorterMixin):
    def __init__(self, parent, *args, **kwargs):
        ...
        self._colNames = None
        self._multiChoices = kwargs.get('multiChoices', None)
        self._showHead = kwargs.get('showHead', True)
        self._choices = kwargs.get('choices', [' '])
        self._lastinsertionpoint = 0
        self._hideOnNoMatch = True
        self._selectCallback = None
        self._entryCallback = None
        ...
        if not (self._showHead and self._multiChoices):
        ...
        #load the data
        if self._multiChoices:
            self.SetMultipleChoices(self._multiChoices, colSearch=0, colFetch=-1)
        else:
